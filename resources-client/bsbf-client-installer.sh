...
echo "Installing bash, curl, fping, jq, lighttpd, mptcpize, and usb-modeswitch."
apt-get update >/dev/null
apt-get install -y bash curl fping jq lighttpd mptcpize usb-modeswitch >/dev/null

echo "Installing bsbf-bonding."
curl -s $BSBF_RESOURCES/resources-client/bsbf-bonding -o /usr/local/sbin/bsbf-bonding
chmod +x /usr/local/sbin/bsbf-bonding

echo "Installing bsbf-client-web."
...
echo ReadWritePaths=/usr/local/etc/bsbf /usr/local/etc/xray >> /usr/lib/systemd/system/lighttpd-bsbf-client-web.service

echo "Installing bsbf-mptcp."
...
curl -s $BSBF_RESOURCES/resources-client/xray.json -o /usr/local/etc/xray-bsbf-bonding/config.json
...
curl -s $BSBF_RESOURCES/resources-client/99-bsbf-bonding.conf -o /etc/systemd/system/xray-bsbf-bonding.service.d/99-bsbf-bonding.conf
...
echo "Enabling bonding."
bsbf-bonding --enable >/dev/null 2>&1

# Enable and (re)start lighttpd-bsbf-client-web systemd service.
systemctl enable --quiet lighttpd-bsbf-client-web
systemctl restart lighttpd-bsbf-client-web

echo "Installation successful. Head to http://localhost:8080/ to access the BSBF Client Monitor."
